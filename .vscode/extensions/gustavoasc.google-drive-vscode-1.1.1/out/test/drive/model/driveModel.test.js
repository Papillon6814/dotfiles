...
describe('Drive model operations', () => {
    ...
    it('List all files and folders from inexisting parent', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        chai_1.expect(files.length).to.equal(0);
    }));
    ...
    it('Download file', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        fs.readFile(destination, (err, data) => {
            if (err)
                assert_1.fail(err);
            chai_1.expect(data.toString()).to.equal('done writing data');
    ...
    it('Download file on invalid location', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            yield model.downloadFile('1C7udIKXCkxsvjO37gCBpfzrHihn9wocz', destination);
            assert_1.fail('Should have rejected');
        }
        catch (error) {
            chai_1.expect(error).to.equal(`ENOENT: no such file or directory, open '/???????///\\\\////invalid-destination/file.txt'`);
        }
    }));
    it('Rename file', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        chai_1.expect(driveFile === null || driveFile === void 0 ? void 0 : driveFile.name).to.equal('novoNome.png');
    }));
});
function compareFileNames(files, names) {
    chai_1.expect(files.length).to.equal(names.length);
    for (let i = 0; i < files.length; i++) {
        chai_1.expect(files[i].name).to.equal(names[i]);
    }
}
function compareParent(files, parent) {
    for (let i = 0; i < files.length; i++) {
        chai_1.expect(files[i].parent).to.equal(parent);
    }
}
class MockFileProvider {
    ...
    createFolder(parentFolderId, folderName) {
        return new Promise((resolve) => {
            chai_1.expect(parentFolderId).to.equal('abcdefghi');
    ...
    uploadFile(parentFolderId, fullFilePath, basename, mimeType) {
        return new Promise((resolve) => {
            switch (this.counter++) {
                case 0:
                    chai_1.expect(parentFolderId).to.equal('abcdefghi');
                    chai_1.expect(fullFilePath).to.equal(path.join(__dirname, 'mockFile.txt'));
                    chai_1.expect(basename).to.equal('mockFile.txt');
                    chai_1.expect(mimeType).to.equal('text/plain');
                    break;
                case 1:
                    chai_1.expect(parentFolderId).to.equal('abcdefghi');
                    chai_1.expect(fullFilePath).to.equal(path.join(__dirname, 'mockPdf.pdf'));
                    chai_1.expect(basename).to.equal('mockPdf.pdf');
                    chai_1.expect(mimeType).to.equal('application/pdf');
                    break;
                case 2:
                    chai_1.expect(parentFolderId).to.equal('abcdefghi');
                    chai_1.expect(fullFilePath).to.equal(path.join(__dirname, 'mockImage.jpg'));
                    chai_1.expect(basename).to.equal('mockImage.jpg');
                    chai_1.expect(mimeType).to.equal('image/jpeg');
                    break;
                case 3:
                    chai_1.expect(parentFolderId).to.equal('abcdefghi');
                    chai_1.expect(fullFilePath).to.equal(path.join(__dirname, 'mockImage.png'));
                    chai_1.expect(basename).to.equal('mockImage.png');
                    chai_1.expect(mimeType).to.equal('image/png');
                    break;
                case 4:
                    chai_1.expect(parentFolderId).to.equal('abcdefghi');
                    chai_1.expect(fullFilePath).to.equal(path.join(__dirname, 'mockWithoutExt'));
                    chai_1.expect(basename).to.equal('mockWithoutExt');
                    chai_1.expect(mimeType).to.equal('text/plain');
                    break;
            }
    ...
    retrieveFileContentStream(fileId) {
        return new Promise((resolve, reject) => {
            chai_1.expect(fileId).to.equal('1C7udIKXCkxsvjO37gCBpfzrHihn9wocz');
    ...
    renameFile(fileId, newName) {
        return new Promise((resolve) => {
            chai_1.expect(fileId).to.equal('1C7udIKXCkxsvjO37gCBpfzrHihn9wocz');
            chai_1.expect(newName).to.equal('novoNome.png');
            resolve();
        });
    }
}
