            ...
            decode: (buffer, options) => {
                try {
                    if (buffer instanceof Buffer) {
                        ...
                        return Promise.resolve(JSON.parse(new util__default['default'].TextDecoder(options.charset).decode(buffer)));
                    }
                }
                catch (err) {
                    return Promise.reject(err);
...
exports.ParameterStructures = messages.ParameterStructures;
...
exports.AbstractMessageReader = messageReader.AbstractMessageReader;
exports.ReadableStreamMessageReader = messageReader.ReadableStreamMessageReader;

exports.MessageWriter = messageWriter.MessageWriter;
exports.AbstractMessageWriter = messageWriter.AbstractMessageWriter;
exports.WriteableStreamMessageWriter = messageWriter.WriteableStreamMessageWriter;

exports.ConnectionStrategy = connection.ConnectionStrategy;
exports.ConnectionOptions = connection.ConnectionOptions;
exports.NullLogger = connection.NullLogger;
exports.createMessageConnection = connection.createMessageConnection;
exports.ProgressType = connection.ProgressType;
exports.Trace = connection.Trace;
exports.TraceFormat = connection.TraceFormat;
exports.SetTraceNotification = connection.SetTraceNotification;
exports.LogTraceNotification = connection.LogTraceNotification;
exports.ConnectionErrors = connection.ConnectionErrors;
exports.ConnectionError = connection.ConnectionError;
exports.CancellationReceiverStrategy = connection.CancellationReceiverStrategy;
exports.CancellationSenderStrategy = connection.CancellationSenderStrategy;
exports.CancellationStrategy = connection.CancellationStrategy;
...
function generateRandomPipeName() {
    const randomSuffix = crypto__default['default'].randomBytes(21).toString('hex');
    if (process.platform === 'win32') {
        return `\\\\.\\pipe\\vscode-jsonrpc-${randomSuffix}-sock`;
    }
    else {
        // Mac/Unix: use socket file
        return path__default['default'].join(os__default['default'].tmpdir(), `vscode-${randomSuffix}.sock`);
    }
}
...
function createClientPipeTransport(pipeName, encoding = 'utf-8') {
    ...
    return new Promise((resolve, reject) => {
        let server = net__default['default'].createServer((socket) => {
            server.close();
            connectResolve([
...
function createServerPipeTransport(pipeName, encoding = 'utf-8') {
    const socket = net__default['default'].createConnection(pipeName);
    return [
        new SocketMessageReader(socket, encoding),
        new SocketMessageWriter(socket, encoding)
    ];
...
function createClientSocketTransport(port, encoding = 'utf-8') {
    ...
    return new Promise((resolve, reject) => {
        const server = net__default['default'].createServer((socket) => {
            server.close();
            connectResolve([
                new SocketMessageReader(socket, encoding),
                new SocketMessageWriter(socket, encoding)
...
function createServerSocketTransport(port, encoding = 'utf-8') {
    const socket = net__default['default'].createConnection(port, '127.0.0.1');
    return [
        new SocketMessageReader(socket, encoding),
        new SocketMessageWriter(socket, encoding)
    ];
...
(function (SemanticTokensRefreshNotification) {
    ...
    SemanticTokensRefreshNotification.type = new messages$1.ProtocolNotificationType0(SemanticTokensRefreshNotification.method);
...
exports.CallHierarchyIncomingCallsRequest = protocol_callHierarchy.CallHierarchyIncomingCallsRequest;
exports.CallHierarchyOutgoingCallsRequest = protocol_callHierarchy.CallHierarchyOutgoingCallsRequest;
exports.CallHierarchyPrepareRequest = protocol_callHierarchy.CallHierarchyPrepareRequest;

exports.SemanticTokenTypes = protocol_semanticTokens.SemanticTokenTypes;
exports.SemanticTokenModifiers = protocol_semanticTokens.SemanticTokenModifiers;
exports.SemanticTokens = protocol_semanticTokens.SemanticTokens;
exports.TokenFormat = protocol_semanticTokens.TokenFormat;
exports.SemanticTokensRequest = protocol_semanticTokens.SemanticTokensRequest;
exports.SemanticTokensDeltaRequest = protocol_semanticTokens.SemanticTokensDeltaRequest;
exports.SemanticTokensRangeRequest = protocol_semanticTokens.SemanticTokensRangeRequest;
exports.SemanticTokensRefreshNotification = protocol_semanticTokens.SemanticTokensRefreshNotification;
/**
...
exports.createProtocolConnection = connection$1.createProtocolConnection;
...
function createConverter(uriConverter) {
    ...
    function asDiagnostic(diagnostic) {
        let result = new vscode__default['default'].Diagnostic(asRange(diagnostic.range), diagnostic.message, asDiagnosticSeverity(diagnostic.severity));
    ...
    function asHoverContent(value) {
        if (is.string(value)) {
            return new vscode__default['default'].MarkdownString(value);
        }
        else if (CodeBlock.is(value)) {
            let result = new vscode__default['default'].MarkdownString();
            return result.appendCodeblock(value.value, value.language);
        }
        else if (Array.isArray(value)) {
            let result = [];
            for (let element of value) {
                let item = new vscode__default['default'].MarkdownString();
            ...
            switch (value.kind) {
                case main$2.MarkupKind.Markdown:
                    return new vscode__default['default'].MarkdownString(value.value);
                case main$2.MarkupKind.PlainText:
                    result = new vscode__default['default'].MarkdownString();
    ...
    function asCodeAction(item) {
        ...
        let result = new vscode__default['default'].CodeAction(item.title);
    ...
    function asWorkspaceEdit(item) {
        ...
        let result = new vscode__default['default'].WorkspaceEdit();
    ...
    function asCallHierarchyItem(item) {
        ...
        let result = new vscode__default['default'].CallHierarchyItem(asSymbolKind(item.kind), item.name, item.detail || '', asUri(item.uri), asRange(item.range), asRange(item.selectionRange));
...
class ProgressPart {
    ...
    begin(params) {
        let location = params.cancellable ? vscode__default['default'].ProgressLocation.Notification : vscode__default['default'].ProgressLocation.Window;
        vscode__default['default'].window.withProgress({ location, cancellable: params.cancellable, title: params.title }, async (progress, cancellationToken) => {
...
class DefaultErrorHandler {
    ...
    closed() {
        ...
        if (this.restarts.length < 5) {
            ...
            if (diff <= 3 * 60 * 1000) {
                vscode__default['default'].window.showErrorMessage(`The ${this.name} server crashed 5 times in the last 3 minutes. The server will not be restarted.`);
...
class DidOpenTextDocumentFeature extends DocumentNotifiactions {
    constructor(client, _syncedDocuments) {
        super(client, vscode__default['default'].workspace.onDidOpenTextDocument, main$2.DidOpenTextDocumentNotification.type, client.clientOptions.middleware.didOpen, (textDocument) => client.code2ProtocolConverter.asOpenTextDocumentParams(textDocument), DocumentNotifiactions.textDocumentFilter);
...
class DidCloseTextDocumentFeature extends DocumentNotifiactions {
    constructor(client, _syncedDocuments) {
        super(client, vscode__default['default'].workspace.onDidCloseTextDocument, main$2.DidCloseTextDocumentNotification.type, client.clientOptions.middleware.didClose, (textDocument) => client.code2ProtocolConverter.asCloseTextDocumentParams(textDocument), DocumentNotifiactions.textDocumentFilter);
        this._syncedDocuments = _syncedDocuments;
    }
    get messages() {
...
class DidSaveTextDocumentFeature extends DocumentNotifiactions {
    constructor(client) {
        super(client, vscode__default['default'].workspace.onDidSaveTextDocument, main$2.DidSaveTextDocumentNotification.type, client.clientOptions.middleware.didSave, (textDocument) => client.code2ProtocolConverter.asSaveTextDocumentParams(textDocument, this._includeText), DocumentNotifiactions.textDocumentFilter);
...
class DocumentSymbolFeature extends TextDocumentFeature {
    ...
    registerLanguageProvider(options) {
        ...
        return [vscode__default['default'].languages.registerDocumentSymbolProvider(options.documentSelector, provider), provider];
    }
}
...
class CodeLensFeature extends TextDocumentFeature {
    ...
    registerLanguageProvider(options) {
        ...
        return [vscode__default['default'].languages.registerCodeLensProvider(options.documentSelector, provider), provider];
    }
}
    ...
    register(_message, data) {
        ...
        if (data.registerOptions.commands) {
            ...
            for (const command of data.registerOptions.commands) {
                disposeables.push(vscode__default['default'].commands.registerCommand(command, (...args) => {
...
class BaseLanguageClient {
    ...
    handleFailedRequest(type, error, defaultValue) {
        ...
        if (error instanceof main$2.ResponseError) {
            if (error.code === main$2.ErrorCodes.RequestCancelled) {
                throw this.makeCancelError();
            }
            else if (error.code === main$2.ErrorCodes.ContentModified) {
                return defaultValue;
            }
        }
...
class SemanticTokensFeature extends client.TextDocumentFeature {
    constructor(client) {
        super(client, main$2.SemanticTokensRequest.type);
    }
    fillClientCapabilities(cap) {
        const capabilites = cap;
        ...
        capability.tokenTypes = [
            ...
            main$2.SemanticTokenTypes.member,
    ...
    initialize(capabilities, documentSelector) {
        let client = this._client;
        client.onNotification(main$2.SemanticTokensRefreshNotification.type, () => {
...
function terminate(process, cwd) {
    if (isWindows) {
        try {
            ...
            child_process__default['default'].execFileSync('taskkill', ['/T', '/F', '/PID', process.pid.toString()], options);
            return true;
        }
        catch (err) {
            return false;
        ...
        try {
            ...
            var result = child_process__default['default'].spawnSync(cmd, [process.pid.toString()]);
...
Object.defineProperty(exports, "__esModule", { value: true });
const cp = child_process__default['default'];
...
class LanguageClient extends commonClient.CommonLanguageClient {
    ...
    checkVersion() {
        let codeVersion = semver.parse(vscode__default['default'].version);
        if (!codeVersion) {
...
exports.memoryUsage = new main$3.RequestType("rust-analyzer/memoryUsage");
exports.status = new main$3.NotificationType("rust-analyzer/status");
exports.reloadWorkspace = new main$3.RequestType("rust-analyzer/reloadWorkspace");
...
exports.resolveCodeAction = new main$3.RequestType('experimental/resolveCodeAction');
...
function assert(condition, explanation) {
    try {
        assert__default['default'].strict(condition, explanation);
    }
    catch (err) {
...
exports.log = new class {
    ...
    stringify(val) {
        if (typeof val === "string")
            return val;
        return util__default['default'].inspect(val, {
            colors: false,
            depth: 6,
        });
    }
...
async function sendRequestWithRetry(client, reqType, param, token) {
    ...
    for (const delay of [40, 160, 640, 2560, 10240, null]) {
        try {
            ...
            if (error.code === node$2.ErrorCodes.RequestCancelled) {
                throw error;
            }
            if (error.code !== node$2.ErrorCodes.ContentModified) {
...
function isValidExecutable(path) {
    ...
    const res = child_process__default['default'].spawnSync(path, ["--version"], { encoding: 'utf8' });
...
async function applySnippetWorkspaceEdit(edit) {
    ...
    const editor = vscode__default['default'].window.visibleTextEditors.find((it) => it.document.uri.toString() === uri.toString());
    if (!editor)
        return;
...
class Cargo {
    ...
    runCargo(cargoArgs, onStdoutJson, onStderrString) {
        return new Promise((resolve, reject) => {
            const cargo = child_process__default['default'].spawn(cargoPath(), cargoArgs, {
...
async function buildCargoTask(target, definition, name, args, customRunner, throwOnError = false) {
    ...
    if (!exec) {
        exec = new vscode__default['default'].ShellExecution(toolchain.cargoPath(), args, definition);
    }
    ...
    const tdcp = new class {
        ...
        provideTextDocumentContent(_uri) {
            if (!vscode__default['default'].window.activeTextEditor)
                return '';
            return ctx.client.sendRequest(lsp_ext.analyzerStatus, null);
        }
        get onDidChange() {
...
function memoryUsage(ctx) {
    const tdcp = new class {
        ...
        provideTextDocumentContent(_uri) {
            if (!vscode__default['default'].window.activeTextEditor)
                return '';
            return ctx.client.sendRequest(lsp_ext.memoryUsage, null).then((mem) => {
...
function serverVersion(ctx) {
    return async () => {
        const { stdout } = child_process__default['default'].spawnSync(ctx.serverPath, ["--version"], { encoding: "utf8" });
...
function reloadWorkspace(ctx) {
    return async () => ctx.client.sendRequest(lsp_ext.reloadWorkspace, null);
}
...
function resolveCodeAction(ctx) {
    const client = ctx.client;
    return async (params) => {
        const item = await client.sendRequest(lsp_ext.resolveCodeAction, params);
        if (!item) {
            return;
        }
...
function createHintStyle(hintKind) {
    const [pos, render] = {
        type: ["after", (label) => `: ${label}`],
        parameter: ["before", (label) => `${label}: `],
        chaining: ["after", (label) => `: ${label}`],
    }[hintKind];
...
async function fetchRelease(releaseTag) {
    ...
    const response = await fetch$1(requestUrl, { headers: { Accept: "application/vnd.github.v3+json" } });
    if (!response.ok) {
...
async function download(opts) {
    ...
    const randomHex = crypto__default['default'].randomBytes(5).toString("hex");
...
async function patchelf(dest) {
    await vscode__default['default'].window.withProgress({
        ...
        await new Promise((resolve, reject) => {
            var _a, _b;
            const handle = child_process__default['default'].exec(`nix-build -E - --arg src '${origFile}' -o ${dest}`, (err, stdout, stderr) => {
                if (err != null) {
                    reject(Error(stderr));
                }
                else {
...
async function getServer(config, state) {
    ...
    await fs__default['default'].promises.unlink(dest).catch(err => {
        if (err.code !== "ENOENT")
            throw err;
    });
    await net.download({
    ...
    if (await fs__default['default'].promises.stat("/etc/nixos").then(_ => true).catch(_ => false)) {
        await patchelf(dest);
    }
...
var main$5 = /*@__PURE__*/unwrapExports(main$4);
