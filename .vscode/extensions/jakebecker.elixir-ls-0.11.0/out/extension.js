...
const runTestFromCodeLens_1 = require("./commands/runTestFromCodeLens");
...
function configureManipulatePipes(context, operation) {
    ...
    const disposable = vscode.commands.registerCommand(commandName, async () => {
        ...
        const command = client.initializeResult.capabilities.executeCommandProvider.commands
            .find((c) => c.startsWith('manipulatePipes:'));
...
function configureRunTestFromCodeLens() {
    vscode.commands.registerCommand(commands_1.default.RUN_TEST_FROM_CODELENS, runTestFromCodeLens_1.default);
}
...
function activate(context) {
    ...
    const clientOptions = {
        ...
        synchronize: {
            ...
            fileEvents: [
                vscode_1.workspace.createFileSystemWatcher("**/*.{ex,exs,erl,hrl,yrl,xrl,eex,leex,heex,sface}"),
            ],
        },
    };
...
function getClient(document) {
    ...
    // otherwise they go to first workspace
    // (even if we pass undefined in clientOptions vs will pass first workspace as rootUri/rootPath)
    let folder = vscode_1.workspace.getWorkspaceFolder(document.uri);
    if (!folder) {
        if (vscode_1.workspace.workspaceFolders && vscode_1.workspace.workspaceFolders.length !== 0) {
            // untitled file assigned to first workspace
            folder = vscode_1.workspace.getWorkspaceFolder(vscode_1.workspace.workspaceFolders[0].uri);
