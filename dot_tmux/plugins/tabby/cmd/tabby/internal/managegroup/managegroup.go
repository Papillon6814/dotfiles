...
func renameGroup(configPath, oldName, newName string) error {
	...
	updateWindowGroups(oldName, newName)

	return config.SaveConfig(configPath, cfg)
}

// updateWindowGroups updates the @tabby_group tmux option on all windows
// whose current group matches oldName to newName.
func updateWindowGroups(oldName, newName string) {
	out, err := exec.Command("tmux", "list-windows", "-a", "-F", "#{session_name}:#{window_index} #{@tabby_group}").Output()
	if err != nil {
		return
	}
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		...
		if currentGroup == oldName {
			exec.Command("tmux", "set-window-option", "-t", target, "@tabby_group", newName).Run()
		}
	}
}
