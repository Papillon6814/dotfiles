...
func spawnPaneHeaders(server *daemon.Server, sessionID string, customBorder bool, headerHeightRows int, windows []tmux.Window) {
	...
	// Build a width lookup for all panes (content + system) so we can detect
	// header/content width mismatches after horizontal splits.
	allPaneWidths := make(map[string]int) // paneID -> width
	if widthOut, err := exec.Command("tmux", "list-panes", "-a", "-F",
		"#{pane_id}|||#{pane_width}").Output(); err == nil {
		for _, line := range strings.Split(strings.TrimSpace(string(widthOut)), "\n") {
			if line == "" {
				continue
			}
	...
	if headerOut, err := exec.Command("tmux", "list-panes", "-a", "-F",
		...
		// Dedup: kill extra headers for the same target
		...
		// Width-mismatch fix: after a horizontal split, a header pane may
		// still span the full pre-split width while its content pane is now
		// half-width. Kill the stale header so it gets respawned at the
		// correct width.
		for target, headerPanes := range headersByTarget {
			if len(headerPanes) == 0 {
				continue
			}
			hdrID := headerPanes[0]
			hdrW := allPaneWidths[hdrID]
			tgtW := allPaneWidths[target]
			if hdrW > 0 && tgtW > 0 && hdrW != tgtW {
				logEvent("HEADER_WIDTH_MISMATCH target=%s header=%s hdr_w=%d tgt_w=%d → kill", target, hdrID, hdrW, tgtW)
				markSkipPreserveForWindow(hdrID)
				exec.Command("tmux", "kill-pane", "-t", hdrID).Run()
