...
func (c *Coordinator) ApplyPaneDimming(activeWindowID string) {
	...
	if !cfg.PaneHeader.DimInactive {
		...
		// Reset border style with fixed colors
		c.enforceBorderColors()
		return
	}
	...
	_ = computeDimBG(termBG, opacity) // unused, bg not set to preserve transparency
	...
	for _, p := range panes {
		...
		// Content pane: dim flag only, no bg override (preserve transparency)
		if active {
			exec.Command("tmux", "set-option", "-p", "-u", "-t", p.id, "window-style").Run()
...
// enforceBorderColors sets pane-active-border-style to the fixed golden color
// and pane-border-style to a dimmed version.
func (c *Coordinator) enforceBorderColors() {
	if c.borderStylingBlocked() {
		return
	}
	const activeBorderFg = "#7A1414"
	exec.Command("tmux", "set-option", "-g", "pane-active-border-style", "fg="+activeBorderFg).Run()
	termBG := c.GetTerminalBg()
	dimFg := desaturateHex(activeBorderFg, 0.6, termBG)
	exec.Command("tmux", "set-option", "-g", "pane-border-style", "fg="+dimFg).Run()
}

// applyBorderDim sets pane-active-border-style to the configured color and
...
func (c *Coordinator) applyBorderDim(opacity float64) {
	...
	// Always enforce the active border color
	fgColor := "#7A1414"
	exec.Command("tmux", "set-option", "-g", "pane-active-border-style", "fg="+fgColor).Run()
...
func (c *Coordinator) RefreshWindows() {
	...
	for _, op := range aiToolOps {
		if op.paneID != "" {
			// Pane-level option
			if op.unset {
				tmuxRun("set-option", "-p", "-t", op.paneID, "-u", op.key)
			} else {
				tmuxRun("set-option", "-p", "-t", op.paneID, op.key, op.value)
			}
		} else {
			// Window-level option
			if op.unset {
...
type tmuxSetOption struct {
	windowID string
	paneID   string // if set, uses -p (pane option) instead of -w (window option)
	key      string
...
func (c *Coordinator) processAIToolStates(preloaded *processTree) []tmuxSetOption {
	...
	for i := range c.windows {
		...
		// Auto-clear @tabby_done when pane becomes active (user has seen it)
		for _, pane := range win.Panes {
			if pane.AIDone && pane.Active {
				pending = append(pending, tmuxSetOption{paneID: pane.ID, key: "@tabby_done", unset: true})
			}
		}
	}
...
func (c *Coordinator) applyThemeToTmux() {
	...
	if c.config.PaneHeader.DimInactive {
		...
		// Don't override fg/bg globally — let terminal theme (including transparency) through
		_ = dimColor(baseFg, dimOpacity) // unused — let terminal theme foreground through for all panes
		exec.Command("tmux", "set-option", "-g", "window-style", "default").Run()
		exec.Command("tmux", "set-option", "-g", "window-active-style", "default").Run()
	}
}
	...
	for _, group := range grouped {
		...
		for _, win := range group.Windows {
			...
			if autoBorder || borderFromTab {
				...
				// Fixed border colors per window
				args = append(args, ";", "set-window-option", "-t", fmt.Sprintf(":%d", win.Index),
					"pane-active-border-style", "fg=#7A1414")
				dimBorderFg := desaturateHex("#7A1414", 0.6, c.config.PaneHeader.TerminalBg)
				args = append(args, ";", "set-window-option", "-t", fmt.Sprintf(":%d", win.Index),
					"pane-border-style", "fg="+dimBorderFg)
			}
...
func (c *Coordinator) generateSidebarHeader(width int, clientID string) (string, []daemon.ClickableRegion) {
	...
	// Override header with tmux key-table mode indicator
	if keyTable, err := exec.Command("tmux", "display-message", "-p", "#{client_key_table}").Output(); err == nil {
		kt := strings.TrimSpace(string(keyTable))
		modeMap := map[string]struct{ name, fg, bg string }{
			"root":        {"NORMAL", "#d4c4b0", "#181825"},
			"off":         {"LOCKED", "#11111b", "#f38ba8"},
			"pane":        {"PANE", "#11111b", "#89b4fa"},
			"tab":         {"TAB", "#11111b", "#fab387"},
			"resize":      {"RESIZE", "#11111b", "#f9e2af"},
			"session":     {"SESSION", "#11111b", "#cba6f7"},
			"group":       {"GROUP", "#11111b", "#94e2d5"},
			"tmux_compat": {"TMUX", "#11111b", "#a6e3a1"},
		}
		if mode, ok := modeMap[kt]; ok {
			headerText = mode.name
			headerHeight = 1
			paddingBottom = 0
			centered = true
			activeColor = false
...
func (c *Coordinator) generateMainContent(clientID string, width, height int) (string, []daemon.ClickableRegion) {
	...
	for gi, group := range c.grouped {
		...
		// Check if this group contains the active window
		groupHasActiveWindow := false
		for _, w := range group.Windows {
			if w.ID == clientID {
				groupHasActiveWindow = true
				break
			}
		}
		...
		{
			bg := theme.Bg
			if groupHasActiveWindow && theme.ActiveBg != "" {
				// Use a darkened version of ActiveBg for the group header
				bg = "#5a1210"
				// Gold text for active group (matching active tab/window)
				headerStyle = headerStyle.Foreground(lipgloss.Color("#d4af37"))
			}
		...
		for wi, win := range group.Windows {
			...
			if isTransparent {
				...
				// Active tab uses a color between group (#5A1210) and pane (#B0201E)
				bgColor = "#851917"
				fgColor = theme.ActiveFg
				if fgColor == "" {
					fgColor = theme.Fg
			...
			if ind.Busy.Enabled && win.Busy {
				...
				// Check if any pane has AIDone (unread completion)
				anyDone := false
				for _, p := range win.Panes {
					if p.AIDone {
						anyDone = true
				...
				if anyDone {
					alertStyle := lipgloss.NewStyle().Foreground(lipgloss.Color("#d4af37"))
					alertIcon = alertStyle.Render("✓")
			...
			if len(contentPanes) > 1 && !isWindowCollapsed {
				...
				for pi, pane := range contentPanes {
					...
					} else if pane.AIDone && !pane.Active {
						// AI tool completed, unread
						alertStyle := lipgloss.NewStyle().Foreground(lipgloss.Color("#d4af37"))
						paneAlertIcon = alertStyle.Render("✓")
					...
					if pane.Active && isActive && theme.ActiveBg != "" {
						paneLineBg = theme.ActiveBg
					} else if bgColor != "" {
						paneLineBg = theme.Bg
					} else {
						paneLineBg = theme.Bg
...
func (c *Coordinator) renderClockWidget(width int) string {
	...
	// terminal default after a theme flip. Prefer clock.Bg if set,
	// otherwise fall back to the coordinator's resolved terminal bg.
	if clock.Bg != "" {
	...
	// Helper to render a full-width blank line with the clock background
	...
	timeRightPad := width - timePadding - lipgloss.Width(timeStr)
	if timeRightPad < 0 {
		timeRightPad = 0
	}
	result.WriteString(style.Render(strings.Repeat(" ", timePadding)+timeStr+strings.Repeat(" ", timeRightPad)) + "\n")
	...
	if clock.ShowDate {
		...
		dateRightPad := width - datePadding - lipgloss.Width(dateStr)
		if dateRightPad < 0 {
			dateRightPad = 0
		}
		result.WriteString(style.Render(strings.Repeat(" ", datePadding)+dateStr+strings.Repeat(" ", dateRightPad)) + "\n")
	}
...
func (c *Coordinator) handleSemanticAction(clientID string, input *daemon.InputPayload) bool {
	...
	case "rename_group":
		...
		// Update @tabby_group on all windows that belong to the old group
		if out, err := exec.Command("tmux", "list-windows", "-a", "-F", "#{session_name}:#{window_index} #{@tabby_group}").Output(); err == nil {
			for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
				if line == "" {
					continue
				}
				...
				if len(parts) >= 2 && parts[1] == oldName {
					exec.Command("tmux", "set-window-option", "-t", parts[0], "@tabby_group", newName).Run()
				}
			}
		}
		return true
