...
test -x "$HOME/Code/herdr-agent-quota-multi-account/target/release/herdr-agent-quota" || { echo 'herdr-agent-quota local fork がありません。Rust 1.95 でビルドしてください' >&2; exit 1; }
herdr plugin link "$HOME/Code/herdr-agent-quota-multi-account" --enabled  # Pi kuno/muu の別アカウント利用枠
